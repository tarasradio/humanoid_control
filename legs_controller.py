from leg_ik import calc_leg_joints, check_leg_joints
import numpy as np
import math
...
def calc_legs_swing(body_z):
  ...
  while t < T:
    ...
    j_l = calc_leg_joints(body, pelvis_to_hip, hip_length, shin_length, l_foot)
    j_r = calc_leg_joints(body, -pelvis_to_hip, hip_length, shin_length, r_foot)

    check_leg_joints(j_l)
    check_leg_joints(j_r)
    
    l_leg_joint_positions['joints'].append(j_l)
    r_leg_joint_positions['joints'].append(j_r)
    t += 0.01
